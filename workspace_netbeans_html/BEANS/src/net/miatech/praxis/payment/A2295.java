/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 */
package net.miatech.praxis.payment;

/**
 *
 * @author andrea
 */
public class A2295 {

    public String CCUST = "";
    public String FCARGA = "";
    public String TRANSTYPE = "";
    public String TKTNBR = "";
    public String AUTHCOD = "";
    public String ISSUEDATE = "";
    public String STATNBR = "";
    public String CARDNBR = "";
    public String MERCHNENV = "";
    public double AUTAMOUNT = 0;
    public String PASSNAME = "";
    public String PNR = "";
    public String SCARDN = "";
    public String SAUTHOC = "";
    public String COMPANYID = "";
    public String ARN = "";
    public String CCIA = "";
    public String FORMA = "";
    public String SERIE = "";
    public String SEQ = "";
    public String CHARNBR = "";
    public String SOCIETY = "";
    public String CANAL= "";
    public String SUCMERCH= "";
    public String COREP = "";
    public String CHGDATE = "";
    public String CHGAMOUNT = "";
    public String CHGCURREN = "";
    public String SCARDNCOR = "";
    public String TKTNUMER = "";
    public String PRDA = "";
    public String ADATE = "";
    public String SDATE = "";
    public String SAGENT = "";
    public String STVAL = "";
    public String FCONCEP = "";
    public String SVFOP = "";
    public String USERF = "";
    public String CERROR = "";
    public String PROCESA = "";
    public String SCURRENCY = "";
    public String SCURRENCY1 = "";
    public String SCURRENCY2= "";
    public String SCOUNTRY = "";
    public String DATECH = "";
    public String RATE = "";
    public String FACTORD = "";
    public String TCCOTIND = "";
    public String TCCOTIND2 = "";
    public String TCCOTDIR = "";
    public String TCCOTDIR2 = "";
    public String ACCNUMBER = "";
    public String SOCIETYL = "";
    public double CCAMOUNT = 0;
    public String DBACITY = "";
    public String FTEA = "";
    public String FSELEC = "";
    public String FECSELEC = "";
    public String FRECEP = "";
    public String CASENBR = "";
    public String CARDTYPE = "";
    public String MERCHNREC = "";
    public String MERCHNAME = "";
    public String CURRAUTH = "";
    public double DISAMOUNT = 0;
    public String CURRDISP = "";
    public double IMPAMOUNT = 0;
    public String CURRIMPU = "";
    public double AMOUNT = 0;
    public String CURRAMNT = "";
    public String DUEDATE = "";
    public String MOTIVO = "";
    public String MOTIVOCOD = "";
    public String REFFNBR = "";
    public String STATKT = "";
    public String STATT = "";
    public String strTitulo = "";
    
    public double SVFOPOT = 0;
    public double SVFOPNETR = 0;
    public double TOT_SVFOPOT = 0;
    public double TOT_SVFOPNETR = 0;
    public double TOT_SVFOPCA = 0;
    public double TOT_SVFOPCC = 0;
    public double TOT_SVFOP = 0;
    public double TOT_QTY = 0;
    public double lngTotAmount = 0;
    public double lngTotQty = 0;
    

    public String TKT = "";
    public String CORRL = "";
    public String TDOC = "";
    public String CFUENTE = "";
    public String descTDOC = "";
    public String SUBFTE = "";
    public String SCONSOL = "";
    public String SVFOPCA = "";
    public String SVFOPCC = "";
    public String INVOICE1 = "";
    public String INVOICE0 = "";
    public String SCARDN1 = "";
    public String SCARCOD1 = "";
    public String SCARDN2 = "";
    public double SPAYMENT = 0;



    //A2335
    public int MERCHN = 0;
    public String MERCHNAM = "";
    public double OPEAMOUNT = 0;
    public double IVA = 0;
    public String APLIDATE = "";
    public String NATURE = "";
    public String CONCEPT = "";
    public int QTYTRNX = 0;
    public String REMESA = "";
    public String REMEDATE = "";
    public String REMETIPO = "";
    public String REMEFOLIO = "";
    public String CODEBANK = "";
    public String SCARCOD = "";
    public String AUTHNBR = "";
    public String SQCRFILE = "";
    
    public long lngTotQSETT = 0;
    public long lngTotQTKT = 0;
    public long lngTotQPROC = 0;
    public long lngTotQNPROC = 0;
    public long lngTotQSETTMATCH  = 0;
    public long lngTotQSETTPEND = 0;
    public long lngTotQTKTMATCH = 0;
    public long lngTotQTKTPEND = 0;
    public long lngTotTotalTran = 0;
    public long lngTotTotalProc = 0;
    public long lngQSETT = 0;
    public long lngQTKT = 0;
    public long lngQSETTMATCH = 0;
    public long lngQSETTPEND = 0;
    public long lngQTKTMATCH = 0;
    public long lngQTKTPEND = 0;
    public long lngQPROC = 0;
    public long lngQNPROC = 0;
    public long lngTotalTran = 0;
    public long lngTotalProc = 0;
    
    
    public String USCR = "";
    public String FECR = "";
    public String HOCR = "";
    public String USUP = "";
    public String FEUP = "";
    public String HOUP = "";
}
