 ...
 * @version 1.1a
 */
public class ConnectionPoolOra implements Runnable {
    ...
    static ConnectionPoolOra pool = null;
    
    public static ConnectionPoolOra getConnectionPool(){
        if(pool == null){
            try{
                pool = new ConnectionPoolOra();
            }catch (Exception e){
                System.out.println("The system can't create the ORA Pool TW...");
            }
        }
        return pool;
    }
    
    public ConnectionPoolOra()throws SQLException {
        ResourceBundle rb = ResourceBundle.getBundle("net.miatech.cm.resource.pool");
        
        this.maxConnections = Integer.parseInt(rb.getString("ORA_maximum"));
        this.waitIfBusy = true;
        int initialConnections = Integer.parseInt(rb.getString("ORA_initial"));
        ...
        for(int i=0; i<initialConnections; i++) {
            availableConnections.addElement(Proveedor.getConnectionOracle());
        }
    }
    ...
    public void run() {
        try {
            Connection connection = Proveedor.getConnectionOracle();
            synchronized(this) {
    ...
    public synchronized void free(Connection connection) {
        if(busyConnections.removeElement(connection)){
            ...
            System.out.println("*-*-*-*-*-No encontro a la conexion ORA TW dentro de los busyConnection ");
        notifyAll();
    }
