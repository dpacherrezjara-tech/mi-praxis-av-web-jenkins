...
public class TableMessageDAO {
    ...
    public TableMessageDAO(IServerSession ss) {
        session = ss;
    }
    ...
    public List<A2353Filter> loadPX285SQP00827(A2353Filter filter) throws SQLException, Exception {
        ...
        String SQLCLL01 = "{CALL " + session.getMainLibrary() + ".SQP00827(?,?,?,?,?,?)}";
        ...
        try {
            ...
            while (rst.next()) {
                ...
                bean.RN = rst.getLong("RN");
                bean.CODEM = rst.getString("CODEM").trim();
                bean.DESCR = rst.getString("DESCR").trim();
    ...
    public A2353Filter loadPX285SQP00829(A2353Filter filter) throws SQLException, Exception {
        ...
        String SQLCLL01 = "{CALL " + session.getMainLibrary() + ".SQP00829(?,?)}";
        ...
        try {
            ...
            cstmt01.setString(1, session.getUserView().getCustomerInfo().CCUST);
            cstmt01.setString(2, filter.CODEM.trim());
            ...
            while (rs01.next()) {
                objRtn.CCUST = rs01.getString("CCUST");
                objRtn.CODEM = rs01.getString("CODEM").trim();
                objRtn.DESCR = rs01.getString("DESCR").trim();
