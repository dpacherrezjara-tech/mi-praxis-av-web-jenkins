 ...
 * @author lmendoza
 */
public class SalesAgentControlDAO {
    ...
    public SalesAgentControlDAO(IServerSession ss) {
        session = ss;
    }
...
//        A2354Filter beanDet;
    ...
    public List<A2354Filter> load_MPS115(A2354Filter filter) throws SQLException, Exception {
        ...
        try {
            ...
            cstmt.setString(2, filter.IN_COUNTRY);
            cstmt.setString(3, filter.IN_INVOICE);
            
            cstmt.setString(4, filter.IN_SOURCE);
            cstmt.setString(5, filter.IN_CANAL);
            cstmt.setString(6, filter.IN_ACCREDITATION);
            cstmt.setString(7, filter.IN_RISK);
            ...
            while (rst.next()) {
                ...
                bean.CANAV = rst.getString("CANAV").trim();
                bean.NAGENT = rst.getString("NAGENT").trim();
                bean.TYPEAG = rst.getString("TYPEAG").trim();
                bean.ASTATUS = rst.getString("ASTATUS").trim();
                bean.RSTATUS = rst.getString("RSTATUS").trim();
                bean.SAGECTR = rst.getString("SAGECTR").trim();
                
               bean.MONTHCRE = rst.getInt("MONTHCRE");
                bean.QTYTKCRE = rst.getInt("QTYTKCRE");

                bean.AMOUNCRE = rst.getDouble("AMOUNCRE");
                bean.FMOUNCRE = rst.getDouble("FMOUNCRE");
                bean.DESVICRE = rst.getDouble("DESVICRE");
                bean.ALERTCRE = rst.getDouble("ALERTCRE");

                bean.MONTHCAS = rst.getInt("MONTHCAS");
                bean.QTYTKCAS = rst.getInt("QTYTKCAS");

                bean.AMOUNCAS = rst.getDouble("AMOUNCAS");
                bean.FMOUNCAS = rst.getDouble("FMOUNCAS");
                bean.DESVICAS = rst.getDouble("DESVICAS");
                bean.ALERTCAS = rst.getDouble("ALERTCAS");
    ...
    public List<A2354Filter> loadMPS365(A2354Filter filter) throws SQLException, Exception {
        ...
        try {
            ...
            while (rst.next()) {
                ...
                beanTkt.NAME_AGENT = rst.getString("DESCRIP").trim();
                beanTkt.CANAL = rst.getString("CANAL").trim();
                beanTkt.PSALF = rst.getString("PSALF").trim();
