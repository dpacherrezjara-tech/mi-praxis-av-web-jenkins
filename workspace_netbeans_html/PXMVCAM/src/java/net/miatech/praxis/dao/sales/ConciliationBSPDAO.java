 ...
 * @author lmendoza
 */
public class ConciliationBSPDAO {
    ...
    public ConciliationBSPDAO(IServerSession ss) {
        session = ss;
    }
    ...
    public List<PX038S02A1698Filter> loadPX038S02A1698(PX038S02A1698Filter filter) throws SQLException, Exception {
        List<PX038S02A1698Filter> lstRtn = new ArrayList<>(0);
        PX038S02A1698Filter objRtn;

        CallableStatement cstmt01 = null;
        ResultSet rs01 = null;

        String SQLCLL01 = "{CALL PX038S02A1698(?,?,?,?,?,?,?,?)}";
        Connection cnx = null;
        try {
            ...
            while (rs01.next()) {
                objRtn = new PX038S02A1698Filter();
    ...
    public PX038S04A1698Filter loadPX038S04A1698(PX038S04A1698Filter filter, String strOption) throws SQLException, Exception {
        List<PX038S04A1698Filter> lstRtn = new ArrayList<>(0);
