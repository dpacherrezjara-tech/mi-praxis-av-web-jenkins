...
public class ReconciliationReportDAO {
    ...
    public ReconciliationReportDAO(IServerSession ss) {
        session = ss;
    }
    ...
    public List<A2354Filter> loadPX305SQP00933(A2354Filter filter) throws SQLException, Exception {
        ...
        try {
            ...
            cstmt.setString(3, filter.IN_TO_VALDATE.trim());
            cstmt.setString(4, filter.IN_ACCID.trim());
            cstmt.setString(5, filter.IN_CORREAV.trim());
            cstmt.setString(6, filter.IN_BANDOC.trim());
            cstmt.setString(7, filter.IN_REFER.trim());
            cstmt.setString(8, filter.IN_STATUSAP.trim());
            ...
            while (rst.next()) {
                ...
                bean.ACCPROV = rst.getString("ACCPROV").trim();
                bean.BENCENC = rst.getString("BENCENC").trim();
                bean.SOCIETY = rst.getString("SOCIETY").trim();
                bean.REFER = rst.getString("REFER").trim();
                bean.CLAVE1 = rst.getString("CLAVE1").trim();
                bean.CLAVE3 = rst.getString("CLAVE3").trim();
                ...
                bean.FASE1_TOTAL = rst.getInt("FASE1_TOTAL");
                bean.FASE2_DEBITOS = rst.getInt("FASE2_DEBITOS");
                bean.FASE2_TOTAL = rst.getInt("FASE2_TOTAL");
                bean.TAXES = rst.getInt("TAXES");
                
                bean.PERIODO_REG = rst.getString("PERIODO_REG").trim();
                bean.HEADER_REG = rst.getString("HEADER_REG").trim();
                bean.CORRELATIVO_REG = rst.getString("CORRELATIVO_REG").trim();
                bean.FECSAP_REG = rst.getString("FECSAP_REG").trim();
                bean.STATUSSAP_REG = rst.getString("STATUSSAP_REG").trim();
                bean.PERIODO_DEB = rst.getString("PERIODO_DEB").trim();
                bean.TIPO_DEB = rst.getString("TIPO_DEB").trim();
                bean.SUB_TIPO_DEB = rst.getString("SUB_TIPO_DEB").trim();
                bean.HEADER_DEB = rst.getString("HEADER_DEB").trim();
                bean.CORRELATIVO_DEB = rst.getString("CORRELATIVO_DEB").trim();
                bean.FILENAME_DEB = rst.getString("FILENAME_DEB").trim();
                bean.FECSAP_DEB = rst.getString("FECSAP_DEB").trim();
                bean.STATUSSAP_DEB = rst.getString("STATUSSAP_DEB").trim();
                
                bean.QTY_REJECTED = rst.getInt("QTY_REJECTED");
