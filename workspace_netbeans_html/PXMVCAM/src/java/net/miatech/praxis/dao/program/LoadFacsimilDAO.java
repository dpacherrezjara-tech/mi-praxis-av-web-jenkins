 ...
 * @author gsanchez
 */
public class LoadFacsimilDAO {
    
    private IServerSession session;
    private static final Logger logError = Logger.getLogger("errorLog");
    
    public LoadFacsimilDAO() {
    }
    
    public LoadFacsimilDAO(IServerSession ss) {
        session = ss;
    }
    ...
    public FACSIMILFilter loadARCFacsimil(BSPF104 filter, HashMap hmCiudades) throws SQLException, Exception {
        ...
        A720Filter beanCpn;
        HashMap hmValueA720 = new HashMap<Object, Object>();
    ...
    public FACSIMILFilter loadFacsimileInterlineal(String calfa, BSPF104 filter, HashMap hmCiudades) throws SQLException, Exception {
        ...
        try {
            ...
            strSQL = "{CALL " + session.getMainLibrary() + ".PXFACINTERLINEAL_1(?,?,?,?,?,?,?,?,?)}";
            cnx = session.getCNXIBMDB2().getIBMDB2Connection();
            ...
            cs.setString(4, filter.DPROCE.trim());
            ...
            // REGISTROS BSPF24 ================================================
            if (cs.getString(5) != null) {
                beanFacsimil.AGTN = cs.getString(5).trim();
            }
            // REGISTRO A003 ===================================================
            if (cs.getString(6) != null) {
                beanFacsimil.strNombreAgente = cs.getString(6).trim();
            }
            if (cs.getString(7) != null && cs.getString(8) != null) {
                beanFacsimil.strDirecAgente = cs.getString(7).trim() + " " + cs.getString(8).trim();
            }
            // REGISTRO A005 ===================================================
            if (cs.getString(9) != null) {
                beanFacsimil.strNomAero = cs.getString(9).trim();
            }
            ...
            if (hayData) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    strSQL = "SELECT DFLIGHT, VCPN, FINVO, FCONT, IDCON FROM " + session.getMainLibrary() + ".A1692 WHERE CCUST = '" + ccust.trim()
                            ...
                            + "' AND FORMA = '" + filter.TDNR.trim().substring(3, 7)
                            + "' AND SERIE = '" + filter.TDNR.trim().substring(7, 13)
                            + "' AND CUPON = '" + (i + 1) + "' AND SUBSTR(DFLIGHT, 1, 4) = '" + filter.DPROCE.substring(0, 4).trim() + "' ";
