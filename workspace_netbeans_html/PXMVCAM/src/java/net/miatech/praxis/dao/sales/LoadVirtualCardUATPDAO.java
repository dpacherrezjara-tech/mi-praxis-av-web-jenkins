 ...
 * @author gsanchez
 */
public class LoadVirtualCardUATPDAO {
    ...
    public List<SQP01356Filter> getListVirtualCard(SQP01356Filter filter) throws Exception{
        List<SQP01356Filter> lstRtn = new ArrayList<>(0);
        SQP01356Filter objRtn;
        try {
            strSQL = "{CALL PXCOMM.SQP01487(?,?,?,?,?,?,?,?)}";//" + session.getMainLibrary() + "
            ...
            while (rst.next()) {
                ...
                //objRtn.A2860ID = rst.getString("A2861ID");
                objRtn.A2860INDAC = rst.getString("A2861INDAC").trim();
                objRtn.A2860VCARD = rst.getString("A2861VCARD").trim();
                objRtn.A2860VCARX = rst.getString("A2861VCARX").trim();
                objRtn.A2860EFFST = Functions.getMonthConvertDate(rst.getString("A2861EFFST").trim());
                objRtn.A2860EFFEN = Functions.getMonthConvertDate(rst.getString("A2861EFFEN").trim());
                objRtn.A2860APLYU = rst.getString("A2861APLYU").trim();
                objRtn.A2860APLYB = rst.getString("A2861APLYB").trim();
                objRtn.A2860PRODU = rst.getString("A2861PRODU").trim();
                objRtn.A2860COMNM = rst.getString("A2861COMNM").trim();
                
                objRtn.A2860UINGR = rst.getString("A2861UINGR").trim();
                objRtn.A2860FINGR = Functions.getMonthConvertDate(rst.getString("A2861FINGR").trim());
                objRtn.A2860HINGR = Functions.getMonthConvertDate(rst.getString("A2861HINGR").trim());
                if(filter.A2860APLYB.equals('S')){
                    objRtn.A2860UMODI = Functions.ConvertedTime(rst.getString("A2861UMODI").trim());
                    objRtn.A2860FMODI = Functions.getMonthConvertDate(rst.getString("A2861FMODI").trim());
                    objRtn.A2860HMODI = Functions.ConvertedTime(rst.getString("A2861HMODI").trim());
                }
                objRtn.A2860ERROR = rst.getString("A2861ERROR").trim();
    ...
    public List<SQP01356Filter>  setMantenimientoCARDMCO( SQP01356Filter filter ,String VP_ACTION) {
        ...
        try {
            ...
            cs.setString(12, filter.A2860FINGR);
            cs.setString(13, filter.A2860HINGR);
            cs.setString(14, session.getUserView().getCustomerInfo().USR);
            cs.setString(15, filter.A2860FMODI);
            cs.setString(16, filter.A2860HMODI);
...
//            logError.error("ClassNotFoundException -> User:" + session.getUserView().getUserInfo().USR + " Message: " + e.getMessage() ,e);
    ...
    public Boolean setFillData(SQP01356Filter bean,ArrayList<String[]> lst) throws Exception{
        /*************************/
        List<SQP01356Filter> lstRtn = new ArrayList<>(0);
        String SQLCLL01 = "{CALL PXCOMM.SQP01486(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
        /*************************/
        try {
            cnx = session.getCNXIBMDB2().getIBMDB2Connection(); 
