...
public class TerminalsCatalogDAO {
    ...
    public TerminalsCatalogDAO(IServerSession ss) {
        session = ss;
    }
    ...
    public MPF106Filter loadPX620SQP05107(MPF106Filter filter) throws SQLException, Exception {
        ...
        String SQLCLL01 = "{CALL " + session.getMainLibrary() + ".SQP05107(?,?,?)}";
        ...
        try {
            ...
            cstmt01.setString(2, filter.TERMP.trim());
            cstmt01.setString(3, filter.SAGENT.trim());
            cstmt01.execute();

            rs01 = cstmt01.getResultSet();
            while (rs01.next()) {
                objRtn.CCUST = rs01.getString("CCUST");
                objRtn.TERMP = rs01.getString("TERMP").trim();
