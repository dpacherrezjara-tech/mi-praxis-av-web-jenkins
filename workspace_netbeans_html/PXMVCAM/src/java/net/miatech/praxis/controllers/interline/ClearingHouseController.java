...
import net.miatech.praxis.logic.interline.ClearingHouseLogic;
...
public class ClearingHouseController extends BaseController {
    ...
    private ClearingHouseLogic logic;
    private MasterDAO masterDAO;
    private IMF093Filter filter;
    ...
    String obteinDataCombo(ModelMap map, HttpServletRequest request) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ClearingHouseController.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    String search(ModelMap map, HttpServletRequest request) {
        List<IMF093Filter> lstData;
        filter = new IMF093Filter();
        try {
            ...
            filter.PERNUM = request.getParameter("PERNUM").trim();
            filter.CURRENP = request.getParameter("CURRENP").trim();

            logic = new ClearingHouseLogic();
            logic.setSession(this.serverSession.getServerSession());
            lstData = logic.loadPX197SQP00144(filter);
    ...
    @RequestMapping(value = "drillDownByCia")
    public @ResponseBody
    String drillDownByCia(ModelMap map, HttpServletRequest request) {
        List<IMF093Filter> lstData;
        filter = new IMF093Filter();
        try {
            filter.FINVOICE = request.getParameter("FINVOICE").trim();
            filter.CCIA = request.getParameter("CCIA").trim();
            filter.PERNUM = request.getParameter("PERNUM").trim();
            filter.CURRENP = request.getParameter("CURRENP").trim();
            ...
            logic = new ClearingHouseLogic();
            logic.setSession(this.serverSession.getServerSession());
            lstData = logic.loadPX197SQP00145(filter);
