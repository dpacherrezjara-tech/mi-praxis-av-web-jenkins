...
import net.miatech.praxis.logic.discharges.ObservedCouponsLogic;
...
@Controller
@Scope("request")
@RequestMapping("/ObservedCoupons")
public class ObservedCouponsController extends BaseController {

    private static final Logger logError = Logger.getLogger("errorLog");
    private ObservedCouponsLogic logic;

    @RequestMapping(method = RequestMethod.POST)
    public String index(ModelMap map) {
        map.put("vp_serverDate", Functions.getFechaActual());
        map.put("vp_serverTime", Functions.getHoraActual());
        return "discharges/ObservedCoupons/form_index";
    }

    @RequestMapping(value = "search")
    public @ResponseBody
    String search(ModelMap map, HttpServletRequest request) {
        System.out.println("-------------- ObservedCoupons : Search-------------");
        map.put("success", true);
        List<PX544S01A3963Filter> lst = this.getList(request, false);
    ...
    public List<PX544S01A3963Filter> getList(HttpServletRequest request, Boolean bExcel) {
        logic = new ObservedCouponsLogic();
        List<PX544S01A3963Filter> lst = new ArrayList<>(0);
        PX544S01A3963Filter filter = new PX544S01A3963Filter();
    ...
    void GetXLSX(HttpServletRequest request, HttpServletResponse response) {
        System.out.println("ObservedCoupons : getXLSX");

        String fileNameDownload = String.format("ObservedCoupons - " + Functions.getFechaActual() + ".xlsx", UUID.randomUUID().toString().toLowerCase());

        try {
            Workbook workbook;
            File file = File.createTempFile(fileNameDownload, ".xlsx");
            List<PX544S01A3963Filter> listaData = this.getList(request, true);

            System.out.println("Tamaño de lista devuelta : " + listaData.size());

            workbook = new XSSFWorkbook();
            Sheet sheet = workbook.createSheet("ObservedCoupons");
            ...
            CH1_06.setCellValue("Final Status");
            CH1_07.setCellValue("Source");
            CH1_08.setCellValue("IATA");
            CH1_09.setCellValue("Country");
            ...
            while (iter.hasNext()) {
                ...
                rcell6.setCellValue(listaData.get(vi).USUP);
                rcell7.setCellValue(listaData.get(vi).FTE);
                rcell8.setCellValue(listaData.get(vi).AGTIA);
                rcell9.setCellValue(listaData.get(vi).CDOC);
                rcell10.setCellValue(listaData.get(vi).PSVVTA);
                ...
                rcell16.setCellValue(listaData.get(vi).VCPN);
                rcell17.setCellValue(listaData.get(vi).COMISI);
                rcell18.setCellValue(listaData.get(vi).SCOMISI);
                rcell19.setCellValue(listaData.get(vi).YQ);
                iter.next();
                ++vi;
                ++vj;
            }
