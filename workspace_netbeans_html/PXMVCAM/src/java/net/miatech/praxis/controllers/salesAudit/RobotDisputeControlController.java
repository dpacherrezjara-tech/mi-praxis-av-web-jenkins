...
import net.miatech.praxis.logic.salesAudit.RobotDisputeControlLogic;
...
@Controller
@Scope("request")
@RequestMapping("/RobotDisputeControl")
public class RobotDisputeControlController extends BaseController {
    ...
    String SearchDebitos(ModelMap map, HttpServletRequest request) {
        A3268Filter filter = new A3268Filter();
        try {
            ...
            RobotDisputeControlLogic logic = new RobotDisputeControlLogic();
            logic.setSession(this.serverSession.getServerSession());
            List<A3268Filter> lst_search = logic.SearchDebitos(filter);
    ...
    String SearchDebitosDetail(ModelMap map, HttpServletRequest request) {
        ...
        try {
            ...
            RobotDisputeControlLogic logic = new RobotDisputeControlLogic();
            logic.setSession(this.serverSession.getServerSession());
            List<A3268Filter> lst_search = logic.SearchDebitosDetail(filter);
    ...
    void getXLSX(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            RobotDisputeControlLogic logic = new RobotDisputeControlLogic();
            logic.setSession(this.serverSession.getServerSession());
            List<A3268Filter> listaData = logic.SearchDebitosDetail(filter);

            // <editor-fold defaultstate="collapsed" desc="Estilo del Excel">
            Workbook workbook = new XSSFWorkbook();
            Sheet sheet = workbook.createSheet("ControlRobotsDispu");
...
//            headerStyle.setFillForegroundColor(IndexedColors.BLUE_GREY.getIndex());
            ...
            while (iter.hasNext()) {
                ...
                CH_00.setCellValue(listaData.get(vi).A3268BASE);
                CH_01.setCellValue(listaData.get(vi).A3268MODO);
                CH_02.setCellValue(listaData.get(vi).A3268FREGI);
                CH_03.setCellValue(listaData.get(vi).A3268PAIS);
                CH_04.setCellValue(listaData.get(vi).A3268NMEMO);

                CH_05.setCellValue(listaData.get(vi).A3268FDISP);
                CH_06.setCellValue(listaData.get(vi).A3268FBCP);
                CH_07.setCellValue(listaData.get(vi).A3268AREA);
                CH_08.setCellValue(listaData.get(vi).A3268TYPE);
                CH_09.setCellValue(listaData.get(vi).A3268RAUDI);
                CH_10.setCellValue(listaData.get(vi).A3268CANT);
                CH_11.setCellValue(listaData.get(vi).A3268DIAS);
                CH_12.setCellValue(listaData.get(vi).A3268FLAG);
