...
import net.miatech.praxis.logic.interline.ISIDECControlLogic;
...
@Controller
@Scope("request")
@RequestMapping("/ISIDECControl")
public class ISIDECControlController extends BaseController {

    private static final Logger logError = Logger.getLogger("errorLog");
    private ISIDECControlLogic logic;
    ...
    String search(ModelMap map, HttpServletRequest request) {
        ...
        try {
            ...
            filter.TTRAN = request.getParameter("TTRAN");
            filter.PERNUM = request.getParameter("PERNUM");
            filter.BNUMBER = request.getParameter("BNUMBER");
            ...
            logic = new ISIDECControlLogic();
            logic.setSession(this.serverSession.getServerSession());
            if (filter.TTRAN.equals("OB")) {
                filter.BDAIR = request.getParameter("BDAIR");
                
                listaData = logic.loadPX195S01SFI010_3(filter);
            } else {
                filter.BAIR = request.getParameter("BAIR");
                
                listaData = logic.loadPX195S01SFI010_2(filter);
            }
    ...
    void getXLSX(HttpServletRequest request, HttpServletResponse response) {
        ...
        String fileNameDownload = String.format("IS-IDEC Control - " + Functions.getFechaActual() + ".xlsx", UUID.randomUUID().toString().toLowerCase());
...
//            filter.BNUMBER = request.getParameter("BNUMBER");
            ...
            Sheet sheet = workbook.createSheet("IS-IDEC Control");
