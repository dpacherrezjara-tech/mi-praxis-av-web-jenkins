...
import net.miatech.praxis.logic.salesAudit.RobotdisputeMyarcFormLogic;
...
public class RobotdisputeMyarcFormController extends BaseController {
    ...
    private RobotdisputeMyarcFormLogic logic;

    @RequestMapping(value = "SearchDebitos")
    public @ResponseBody
    String SearchDebitos(ModelMap map, HttpServletRequest request) {
        A4139Filter filter = new A4139Filter();
        try {
            Functions.msjConsola("PRAXIS", this.serverSession.getServerSession().getUserView().getUserInfo().USR, getClass().getSimpleName() + " : " + Thread.currentThread().getStackTrace()[1].getMethodName());
            filter = new Gson().fromJson(request.getParameter("beanString"), filter.getClass());

            logic = new RobotdisputeMyarcFormLogic();
            logic.setSession(this.serverSession.getServerSession());
            List<A4139Filter> lst_search = logic.SearchDisputas(filter);
    ...
    void getXLSX(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logic = new RobotdisputeMyarcFormLogic();
            logic.setSession(this.serverSession.getServerSession());
            List<A4139Filter> listaData = logic.SearchDisputas(filter);

            // <editor-fold defaultstate="collapsed" desc="Estilo del Excel">
            Workbook workbook = new XSSFWorkbook();
            Sheet sheet = workbook.createSheet("RobotControlMyARC");
...
//            headerStyle.setFillForegroundColor(IndexedColors.BLUE_GREY.getIndex());
            ...
            String fileNameDownload = String.format("RobotControlMyARC - " + Functions.getFechaActual() + ".xlsx", UUID.randomUUID().toString().toLowerCase());
...
//            headerStyle.setFillForegroundColor(IndexedColors.BLUE_GREY.getIndex());
            ...
            while (iter.hasNext()) {
                ...
                CH_01.setCellValue(listaData.get(vi).A4139PAIS);
                CH_02.setCellValue(listaData.get(vi).A4139CANTI);
                CH_03.setCellValue(listaData.get(vi).A4139STATO);
                CH_04.setCellValue(listaData.get(vi).A4139ROBOT);
                CH_05.setCellValue(listaData.get(vi).A4139FREVI);
                CH_06.setCellValue(listaData.get(vi).A4139ROBOT);
                CH_07.setCellValue(listaData.get(vi).A4139NMEMO);
                CH_08.setCellValue(listaData.get(vi).A4139TYPE);
                CH_09.setCellValue(listaData.get(vi).A4139AREA);
                CH_10.setCellValue(listaData.get(vi).A4139BASE);
                CH_11.setCellValue(listaData.get(vi).A4139NETO);
                CH_12.setCellValue(listaData.get(vi).A4139NETO);
                CH_13.setCellValue(listaData.get(vi).A4139DIAS);
