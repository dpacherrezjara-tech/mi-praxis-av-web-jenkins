...
import net.miatech.praxis.logic.sales.EstimationReverseProcessLogic;
...
@Controller
@Scope("request")
@RequestMapping("/EstimationReverseProcessSales")
public class EstimationReverseProcessControllerSales extends BaseController {
    ...
    public String index(ModelMap map) {
        ...
        return "sales/EstimationReverseProcessSales/form_index";
    }
    ...
    String search(ModelMap map, HttpServletRequest request) {
        ...
        A2017Filter filter;
        List<A2017Filter> lstData;
        Gson gson = new Gson();

        try {
            Functions.msjConsola("PRAXIS", this.serverSession.getServerSession().getUserView().getUserInfo().USR, getClass().getSimpleName() + " : " + Thread.currentThread().getStackTrace()[1].getMethodName());
            String beanString = request.getParameter("beanString");
            filter = gson.fromJson(beanString, A2017Filter.class);

            logic = new EstimationReverseProcessLogic();
            logic.setSession(this.serverSession.getServerSession());
            lstData = logic.loadPX251S01A2017(filter);
