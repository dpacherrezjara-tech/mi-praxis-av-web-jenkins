...
import net.miatech.praxis.logic.salesAudit.EmailcatalogReportFormLogic;
...
@Controller
@Scope("request")
@RequestMapping("/EmailcatalogReportForm")
public class EmailcatalogReportFormController extends BaseController {

    private static final Logger logError = Logger.getLogger("errorLog");
    private EmailcatalogReportFormLogic logic;

    @RequestMapping(value = "search")
    public @ResponseBody
    String search(ModelMap map, HttpServletRequest request) {
        List<A3903Filter> lst;
        A3903Filter filter = new A3903Filter();

        try {
            logic = new EmailcatalogReportFormLogic();
    ...
    String DeleteADMMANUAL(ModelMap map, HttpServletRequest request) {
        ...
        try {
            ...
            logic = new EmailcatalogReportFormLogic();
            logic.setSession(this.serverSession.getServerSession());
            result = logic.mantenimiento(filter);
    ...
    void getXLSX(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logic = new EmailcatalogReportFormLogic();
            logic.setSession(this.serverSession.getServerSession());
            List<A3903Filter> listaData = logic.search(filter);

            // <editor-fold defaultstate="collapsed" desc="Estilo del Excel">
            Workbook workbook = new XSSFWorkbook();
            Sheet sheet = workbook.createSheet("EmailcatalogReportForm");
...
//            headerStyle.setFillForegroundColor(IndexedColors.BLUE_GREY.getIndex());
            ...
            CH_03.setCellValue("Email Airline");
            CH_04.setCellValue("Email Agency");
            CH_05.setCellValue("Status");
            CH_06.setCellValue("User Created ");
            ...
            while (iter.hasNext()) {
                ...
                CH_02.setCellValue(listaData.get(vi).A3903NOMAGENCY);
                CH_03.setCellValue(listaData.get(vi).A3903CORER);
                CH_04.setCellValue(listaData.get(vi).A3903COREG);
                CH_05.setCellValue(listaData.get(vi).A3903FLAG);
                CH_06.setCellValue(listaData.get(vi).A3903FREGI);
            ...
            String fileNameDownload = String.format("EmailcatalogReportForm - " + Functions.getFechaActual() + ".xlsx", UUID.randomUUID().toString().toLowerCase());
