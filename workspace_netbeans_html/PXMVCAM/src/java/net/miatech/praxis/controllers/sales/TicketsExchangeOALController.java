...
import net.miatech.praxis.logic.sales.TicketsExchangeOALLogic;
...
@Controller
@Scope("request")
@RequestMapping("/TicketsExchangeOAL")
public class TicketsExchangeOALController extends BaseController {

    private static final Logger logError = Logger.getLogger("errorLog");
    private TicketsExchangeOALLogic logic;

    @RequestMapping(method = RequestMethod.POST)
    public String index(ModelMap map) {
        map.put("vp_serverDate", Functions.getFechaActual());
        map.put("vp_serverTime", Functions.getHoraActual());
        return "sales/TicketsExchangeOAL/form_index";
    }

    @RequestMapping(value = "search")
    public @ResponseBody
    String search(ModelMap map, HttpServletRequest request) {
        System.out.println("-------------- TicketsExchangeOAL : Search-------------");
        map.put("success", true);
        List<PX150S01A730Filter> lst = this.getList(request, false);
    ...
    public List<PX150S01A730Filter> getList(HttpServletRequest request, Boolean bExcel) {

        logic = new TicketsExchangeOALLogic();

        List<PX150S01A730Filter> lst = new ArrayList<>(0);
        PX150S01A730Filter filter = new PX150S01A730Filter();
        ...
        try {
            ...
            lst = logic.loadPX150S01A730(filter);
        } catch (Exception e) {
   ...
   @RequestMapping(value = "getXLSX")
    ...
    void GetXLSX(HttpServletRequest request, HttpServletResponse response) {
        System.out.println("TicketsExchangeOAL : getXLSX");

        String fileNameDownload = String.format("TicketsExchangeOAL - " + Functions.getFechaActual() + ".xlsx", UUID.randomUUID().toString().toLowerCase());

        try {

            Workbook workbook;
            File file = File.createTempFile(fileNameDownload, ".xlsx");
            List<PX150S01A730Filter> listaData = this.getList(request, true);

            System.out.println("Tamaño de lista devuelta : " + listaData.size());

            workbook = new XSSFWorkbook();

            Sheet sheet = workbook.createSheet("TicketsExchangeOAL");
            ...
            CH1_01.setCellValue("Invoice Period");
            CH1_02.setCellValue("Procesing Date");
            CH1_03.setCellValue("Source");
            CH1_04.setCellValue("IATA");
            CH1_05.setCellValue("NEW TICKET");           
            CH1_08.setCellValue("OLD TICKET");
            CH1_16.setCellValue("INVOICE");
            CH1_19.setCellValue("Diference Fare");           
            ...
            CH2_05.setCellValue("Air");
            CH2_06.setCellValue("Document");
            CH2_07.setCellValue("Issue Date");
            CH2_08.setCellValue("Air");
            CH2_09.setCellValue("Document");
            CH2_10.setCellValue("Issue Date");
            CH2_11.setCellValue("ETKT");
            CH2_12.setCellValue("CNJ");
            CH2_13.setCellValue("Coupon");
            CH2_14.setCellValue("Curr");
            CH2_15.setCellValue("Fare");
            CH2_16.setCellValue("Date");
            CH2_17.setCellValue("Nbr");
            ...
            while (iter.hasNext()) {
                ...
                rcell0.setCellValue(listaData.get(vi).A1964FCONT);
                rcell1.setCellValue(listaData.get(vi).A1964INVOICE);
                rcell2.setCellValue(listaData.get(vi).A1530FPROC);
                rcell3.setCellValue(listaData.get(vi).A720ORIG);
                rcell4.setCellValue(listaData.get(vi).A720AGENTE);
                rcell5.setCellValue(listaData.get(vi).A720CIAI);
                rcell6.setCellValue(listaData.get(vi).A720DOCUMENT);
                rcell7.setCellValue(listaData.get(vi).A720FECVTA);
                rcell8.setCellValue(listaData.get(vi).A730CIA);
                rcell9.setCellValue(listaData.get(vi).A730DOCUMENT);
                rcell10.setCellValue(listaData.get(vi).A730FECVTA);
                rcell11.setCellValue(listaData.get(vi).A720ETKT);
                rcell12.setCellValue(listaData.get(vi).A730FLAG);
                rcell13.setCellValue(listaData.get(vi).A730CUPON);
                rcell14.setCellValue(listaData.get(vi).A730MONREG);
                rcell15.setCellValue(listaData.get(vi).VALUE);
                rcell16.setCellValue(listaData.get(vi).DES_A050FCONTA);
                rcell17.setCellValue(listaData.get(vi).DES_A050CRTR);
                rcell18.setCellValue(listaData.get(vi).A050ACEPTA);
                rcell19.setCellValue(listaData.get(vi).Diff);
