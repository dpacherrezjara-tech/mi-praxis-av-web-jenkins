...
import net.miatech.praxis.logic.gerencial.BiToolsLogic;
...
@Controller
@Scope("request")
@RequestMapping("/BiTools")
public class BiToolsController extends BaseController {

    private static final Logger logError = Logger.getLogger("errorLog");
    private BiToolsLogic logic;
    ...
    String searchFields(ModelMap map, HttpServletRequest request) {
        System.out.println("-------------- BiTools : searchFields-------------");
...
//
    ...
    String obtainDataCampos(ModelMap map, HttpServletRequest request) {
        System.out.println("-------------- BiTools : obtainDataCampos-------------");
        logic = new BiToolsLogic();
        List<A1248> lstCampos = new ArrayList<>(0);
        try {
            logic.setSession(this.serverSession.getServerSession());
    ...
    String SaveQuery(ModelMap map, HttpServletRequest request) {
        System.out.println("-------------- BiTools : SaveQuery-------------");
        logic = new BiToolsLogic();
        SQP00768 filter = new SQP00768();
        String msj = "";
        try {
    ...
    String obtainListFavoritos(ModelMap map, HttpServletRequest request) {
        System.out.println("-------------- BiTools : obtainListFavoritos-------------");
        logic = new BiToolsLogic();
        List<SQP00768> lstFavoritos = new ArrayList<>(0);
        try {
            logic.setSession(this.serverSession.getServerSession());
    ...
    String obtainDataFavoritos(ModelMap map, HttpServletRequest request) {
        System.out.println("-------------- BiTools : obtainDataFavoritos-------------");
        logic = new BiToolsLogic();
        List<SQP00768> lstFavoritos = new ArrayList<>(0);
        try {
            logic.setSession(this.serverSession.getServerSession());
...
//            response.setHeader("Content-Disposition", "attachment; filename=\"Policy.ZIP\"");
    ...
    String executeValuation(ModelMap map, HttpServletRequest request) {
        System.out.println("-------------- BiTools : executeValuation-------------");
        logic = new BiToolsLogic();
        SQP00768 filter = new SQP00768();
        String msj = "";
        try {
...
//        System.out.println("BiTools : getXLSX");
//
//        String fileNameDownload = String.format("BiTools- " + Functions.getFechaActual() + ".xlsx", UUID.randomUUID().toString().toLowerCase());
//
//        try {
//
//            Workbook workbook;
...
//
//            Sheet sheet = workbook.createSheet("BiTools");
//
