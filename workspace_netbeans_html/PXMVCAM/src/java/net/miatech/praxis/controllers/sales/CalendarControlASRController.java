...
import net.miatech.praxis.logic.sales.CalendarControlASRLogic;
...
@Controller
@Scope("request")
@RequestMapping("/CalendarControlASR")
public class CalendarControlASRController extends BaseController {

    private CalendarControlASRLogic logic;
    private PX111S01A1528Filter filter;
    private SQP00152Filter filter2;
    
    @RequestMapping(value = "/search")
    public @ResponseBody
    String search(ModelMap map, HttpServletRequest request) {
        filter = new PX111S01A1528Filter();
        
        List<PX111S01A1528Filter> oList = new ArrayList<PX111S01A1528Filter>(0);
        ...
        try {
            ...
            logic = new CalendarControlASRLogic();
            logic.setSession((IServerSession) serverSession.getServerSession());
            oList = logic.loadPX111S01A1528(filter);
            ...
            int contMonth = 0;
            int contGeneral = 0;
            String vl_week = oList.get(0).A1528PDIDS;
            for(int i=0;i<oList.size();i++){
                HashMap fechas = new HashMap();
                if(!vl_week.equals(oList.get(i).A1528PDIDS)){
                ...
                switch(oList.get(i).A1528PDIDC){
                    case "2": vl_monday = oList.get(i).A1528PRDA;
                              ...
                              if(Integer.parseInt(oList.get(i).A1528PRDA)<=Integer.parseInt(fechaActual)){
                                vl_monday_St = Style(oList.get(i).A1698_TAPES,oList.get(i).A1698_SALEWO,oList.get(i).A1698_ERRORS);
                              }
                              break;
                    case "3": vl_tuesday = oList.get(i).A1528PRDA;
                              ...
                              break;
                    case "4": vl_wednesday = oList.get(i).A1528PRDA;
                              ...
                              break;
                    case "5": vl_thursday = oList.get(i).A1528PRDA;
                              ...
                              break;
                    case "6": vl_friday = oList.get(i).A1528PRDA;
                              ...
                              break;
                    case "7": vl_saturday = oList.get(i).A1528PRDA;
                              ...
                              break;
                    case "1": vl_sunday = oList.get(i).A1528PRDA;
                ...
                if(contMonth==1 && (!vl_month.equals(oList.get(i).A1528PDIDM) || i == oList.size()-1)){
                ...
                if(!vl_month.equals(oList.get(i).A1528PDIDM)){
                    vl_month = oList.get(i).A1528PDIDM;
                    switch(vl_month){
                        case "01": vl_month_vl = "JAN"; break;
                        case "02": vl_month_vl = "FEB"; break;
                        case "03": vl_month_vl = "MAR"; break;
                        case "04": vl_month_vl = "APR"; break;
                        case "05": vl_month_vl = "MAY"; break;
                        case "06": vl_month_vl = "JUN"; break;
                        case "07": vl_month_vl = "JUL"; break;
                        case "08": vl_month_vl = "AUG"; break;
                        case "09": vl_month_vl = "SEP"; break;
                        case "10": vl_month_vl = "OCT"; break;
                        case "11": vl_month_vl = "NOV"; break;
                        case "12": vl_month_vl = "DEC"; break;
                    }
                }
                
                HashMap qtr = new HashMap();
                if((contGeneral==1 && !vl_qtr.equals(oList.get(i).A1528CUART)) || i == oList.size()-1){
                ...
                if(!vl_qtr.equals(oList.get(i).A1528CUART)){
                    vl_qtr = oList.get(i).A1528CUART;
                }
            }
    ...
    String updateObservation(ModelMap map, HttpServletRequest request) {
        filter2 = new SQP00152Filter();
        try {
            filter2.IN_A1528FPRO = request.getParameter("IN_A1528FPRO").trim();
            filter2.IN_A1528PDIDC = request.getParameter("IN_A1528PDIDC").trim();
            filter2.IN_A1528PRDA = request.getParameter("IN_A1528PRDA").trim();
            filter2.IN_A1698COMEN = request.getParameter("IN_A1698COMEN").trim();
            
            logic = new CalendarControlASRLogic();
            logic.setSession((IServerSession) serverSession.getServerSession());
            logic.setSQP00152(filter2);
    ...
    void getXLSX(HttpServletRequest request, HttpServletResponse response) {
        List<PX111S01A1528Filter> listaData;
        filter = new PX111S01A1528Filter();        
        //String fileNameDownload = String.format("ASR Calendar Control - " + Functions.getFechaActual() + ".xlsx", UUID.randomUUID().toString().toLowerCase());
        String fileNameDownload = String.format("PX111_" + Functions.getFechaActual() + "_ASR_Calendar_Control.xlsx", UUID.randomUUID().toString().toLowerCase());                
        try {
            ...
            logic = new CalendarControlASRLogic();
            logic.setSession((IServerSession) serverSession.getServerSession());
            listaData = logic.loadPX111S01A1528(filter);
...
//            headerStyle.setFillForegroundColor(IndexedColors.BLUE_GREY.getIndex());
            ...
            CH1_02.setCellValue("Week");
            Cell CH1_03 = row.createCell(3);
            CH1_03.setCellValue("Cicle");
            Cell CH1_04 = row.createCell(4);
            CH1_04.setCellValue("Observation");
            ...
            while (iter.hasNext()) {
                //if(Integer.parseInt(listaData.get(vi).A1528PRDA)<=Integer.parseInt(fechaActual)){
                    //if (listaData.get(vi).A1698_TAPES != 1) {
                        ...
                        cell50.setCellValue(listaData.get(vi).A1528PRDA);
                        cell51.setCellValue(listaData.get(vi).A1528FPRO);
                        cell52.setCellValue(listaData.get(vi).A1528PDIDS);
                        cell53.setCellValue(listaData.get(vi).A1528PDIDC);
                        cell54.setCellValue(listaData.get(vi).A1528OBS);
