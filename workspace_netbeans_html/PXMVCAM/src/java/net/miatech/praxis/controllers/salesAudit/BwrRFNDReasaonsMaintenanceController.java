...
import net.miatech.praxis.logic.salesAudit.BwrRFNDReasaonsMaintenanceLogic;
...
@Controller
@Scope("request")
@RequestMapping("/BsplinkReasonsRFND")
public class BwrRFNDReasaonsMaintenanceController extends BaseController {

    private static final Logger logError = Logger.getLogger("errorLog");
    private BwrRFNDReasaonsMaintenanceLogic logic;
    ...
    String SearchRFNDReasaons(ModelMap map, HttpServletRequest request) {
        ...
        try {
            logic = new BwrRFNDReasaonsMaintenanceLogic();
            ...
            filter.A3404FAMIL = request.getParameter("A3404FAMIL").toString().trim();
    ...
    String MantRFNDReasaons(ModelMap map, HttpServletRequest request) {
        ...
        try {
            ...
            BwrRFNDReasaonsMaintenanceLogic logic = new BwrRFNDReasaonsMaintenanceLogic();
            logic.setSession(this.serverSession.getServerSession());
    ...
    void getXLSX(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            logic = new BwrRFNDReasaonsMaintenanceLogic();
            logic.setSession(this.serverSession.getServerSession());
            List<A3404Filter> lst = logic.SearchRFNDReasaons(filter);

            Workbook workbook = new XSSFWorkbook();
            Sheet sheet = workbook.createSheet("Reasons");
            ...
            while (iter.hasNext()) {
                ...
                CH_00.setCellValue(lst.get(vi).A3404CCUST);
                CH_01.setCellValue(lst.get(vi).A3404CODRZ);
                CH_02.setCellValue(lst.get(vi).A3404COMRE);
                CH_03.setCellValue(lst.get(vi).A3404COMES);
                CH_04.setCellValue(lst.get(vi).A3404COMEN);
                CH_05.setCellValue(lst.get(vi).A3404COMPO);
                CH_06.setCellValue(lst.get(vi).A3404COMFR);
