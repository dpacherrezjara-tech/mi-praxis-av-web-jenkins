...
public class MailUtils {
    ...
    public void sendMailWithImg(
            ...
            List<String> Ccpy,
            String mensaje,
            List<MailImagePath> imgPaths,
            List<String> adjuntos) throws Exception {
        String usuario = (String) cs.getPropertySession().get("APP_SERVER_MAIL_EMAIL"); //Correo con el que saldra el email enviado ("from")            
        String password = (String) cs.getPropertySession().get("APP_SERVER_MAIL_PASSWORD");
        Properties props = System.getProperties();
        //Se define el servidor de correos
        props.put("mail.smtp.host", "smtp-mail.outlook.com");
        props.put("mail.smtp.port", "587");
        ...
        if (imgPaths!=null && !imgPaths.isEmpty()) {
            imgPaths.forEach(img->{
                try {
                    ...
                    imagePart.attachFile(img.getPath()); // Cambia por la ruta de tu archivo PNG
                    imagePart.setContentID("<" + img.getCid() + ">");
