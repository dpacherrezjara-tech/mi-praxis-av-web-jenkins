...
Ext.define('Ext.Praxis.controller.payments.RobotConfig.RobotConfigController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.RobotConfigController',
    fecha: new Date(),
    childs: '5',
    bean: '',
    paginActual: '',
    ...
    init: function (view) {
        ...
        this.control({
//            //   -------------------Eventos Genericos --------------------
            '#RobotConfigForm-xpanel': {
                afterrender: this.xpanel_afterrender
            },
            '#RobotConfigForm-btnSearch': {
                click: this.btnSearch_click
            },
            '#RobotConfigForm-btnClear': {
                click: this.btnClear_click
            },
            '#RobotConfigForm-btnExcel': {
                click: this.btnExcel_click
            },
            '#RobotConfigForm-btnFilter': {
                click: this.btnFilter_click
            },
            '#RobotConfigForm-btnAdd': {
                click: this.btnAdd_click
            },
            '#RobotConfigForm-btnAddDetail': {
                click: this.btnAdd_clickDetail
            },
            '#RobotConfigForm-btnBack': {
                click: this.btnBack_click
            },
            '#RobotConfigForm-btn-pag-first': {
                click: this.pagFirst
            },
            '#RobotConfigForm-btn-pag-previous': {
                click: this.pagPrevious
            },
            '#RobotConfigForm-btn-pag-next': {
                click: this.pagNext
            },
            '#RobotConfigForm-btn-pag-last': {
                click: this.pagLast
            }
//            //-----------------Eventos Especificos -------------------    
//
...
//        this.btnSearch_click();
    ...
    setGridData: function () {
        ...
        win.lblUser_toolTip("Estructura: MPFRBTD");
        me.panelActual = '-panelGridData';
        global.selectedChild(me.childs, prototype.id + me.panelActual);
        me.setWidthPie();
...
//                    }
    ...
    winDataEntry: function (action, rec) {
        ...
        Ext.create('Ext.Praxis.view.payments.RobotConfigForm.DataEntry', {
    ...
    btnAdd_clickDetail: function () {
        this.winDataEntryDetail('I');
    },
    onEditDetailClick: function (grid, rowIndex, colIndex) {
        var rec = grid.getStore().getAt(rowIndex);
        this.winDataEntryDetail('U', rec);
    },
    winDataEntryDetail: function (action, rec) {
        action = action === null || action === undefined ? 'U' : action;
        rec = rec === null || rec === undefined ? {} : rec;

        Ext.create('Ext.Praxis.view.payments.RobotConfigForm.DataEntryDetail', {
            id: prototype.id + '-dataEntry',
            params: {
                action: action,
                rec: rec
...
//        console.log('btnFilter_click');
    ...
    gridDet_clickHandler: function (obj, metaData, rowNum, columnNum, obj2, rowData) {

        me.drillDown.push(me.panelActual);
        me.panelActual = '-panelGridDataDetail';
        global.selectedChild(me.childs, prototype.id + me.panelActual);

        this.beanDetail.IN_CODES = rowData.data.CODES;
        
        var titulo = '<center style="font-size:12px;">Carpeta: ' + rowData.data.NAME + '</center>'

        console.log(this.beanDetail);

        me.paramsDetail.beanString = JSON.stringify(this.beanDetail);
        this.setGridDetail(titulo);
    },
    setGridDetail: function (titulo) {
        win.lblUser_toolTip("Estructura: MPFRBTD");
        me.setWidthPie();
        var storeGridDatas = Ext.create('Ext.Praxis.store.payments.GridData', {
            proxy: {
                ...
                load: function (obj) {
                    ...
                    if (obj.data.length === 0) {
                        ...
                        Ext.getCmp(prototype.id + '-gridDataDetail').setTitle(titulo);                        
                    }
                    me.setWidthPie();
                }
            }
