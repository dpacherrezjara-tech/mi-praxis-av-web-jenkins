Ext.define('Ext.Praxis.controller.payments.HeadersReport.SequencesDataEntryController', {
    ...
    me.view.setLoading(true);
        ...
        // === Limpieza de grillas ===
        const gridSequences = Ext.getCmp(prototype.idDEheader + '-gridAccounted');
        const gridDerived = Ext.getCmp(prototype.idDEheader + '-gridDerived');
    ...
    try {
        ...
        const [resSequences, resDerived] = await Promise.all([
            global.callStoreGet('PRAXISMP', 'MPS308', {
                IN_TYPE: 'S',
        ...
        const infoSequences = resSequences.lstRs[0] || [];
        const infoDerived = resDerived.lstRs[0] || [];

        if (gridSequences) {
            if (!gridSequences.getStore()) {
                const store = Ext.create('Ext.data.Store', {
                    fields: [
                        'BANKDOC', 'VALUE_DATE', 'PROCESADOR', 'AFILIADO', 'IATA',
                ...
                gridSequences.getStore().loadData(infoSequences);
            }
            gridSequences.getView().refresh(); // 🔹 fuerza repintado del grid
        };
        if (gridDerived) {
            if (!gridDerived.getStore()) {
                const store = Ext.create('Ext.data.Store', {
                    fields: [
                ...
                gridDerived.getStore().loadData(infoDerived);
            }
            gridDerived.getView().refresh(); // 🔹 fuerza repintado del grid
            
        };
        const tabDerived = Ext.getCmp(prototype.idDEheader + '-tabDerived');    
        if (infoDerived.length > 0){
