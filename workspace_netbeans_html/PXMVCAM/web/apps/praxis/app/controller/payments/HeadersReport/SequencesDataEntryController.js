Ext.define('Ext.Praxis.controller.payments.HeadersReport.SequencesDataEntryController', {
    ...
    loadStores: function () {
        ...
        if (me.dataSequences && me.dataSequences.length > 0)
            tabSequences.setDisabled(false);
        else
            tabSequences.setDisabled(true);
        if (me.dataDerived && me.dataDerived.length > 0)
            tabDerived.setDisabled(false);
        else
            tabDerived.setDisabled(true);
        if (me.dataRejections && me.dataRejections.length > 0)
            tabRejections.setDisabled(false);
        else
            tabRejections.setDisabled(true);

        // enfocar tab con data
        if (me.dataSequences && me.dataSequences.length > 0) {
            if (mainTabPanel && tabSequences)
                mainTabPanel.setActiveTab(tabSequences); //  Enfocar el tabSequences
        } else if (me.dataDerived && me.dataDerived.length > 0) {
            if (mainTabPanel && tabDerived)
                mainTabPanel.setActiveTab(tabDerived); //  Enfocar el tabDerived
        } else if (me.dataRejections && me.dataRejections.length > 0) {
            if (mainTabPanel && tabRejections)
                mainTabPanel.setActiveTab(tabRejections); //  Enfocar el tabDerived
        } else {
            if (mainTabPanel && tabSequences)
                mainTabPanel.setActiveTab(tabSequences); //  Enfocar el tabSequences 
        }
...
//                                        console.log("new dataSequences", me.dataSequences);
    ...
    maintenanceAccounting: async function (params) {
        const me = this;
        const rejectData = Ext.getCmp(prototype.idDEsequence + '-gridRejections').getStore().getData()
                .items.map(x => ({...x.data, IDCONT: params.idCont}));
        let option = '';
