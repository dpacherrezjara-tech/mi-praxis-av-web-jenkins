...
//        beanDetAll.IN_CONSULTA = "1";
        ...
        var fte = Ext.getCmp(prototype.id + '-cmbFUENTE').getValue();
        if (fte === 'All') {
            beanDetAll.IN_FUENTE = '';
        } else {
            beanDetAll.IN_FUENTEE = fte;
        }
    ...
    searchDetAllFilter_clickHandler: function (obj, metaData, rowNum, columnNum, obj2, rowData) {
        ...
        if (vista === 'C') {
            ...
            beanDetAll.IN_FECHA_FROM = Ext.getCmp(prototype.id + '-cmbDateFromYear').getValue() +
                    Ext.getCmp(prototype.id + '-cmbDateFromMonth').getValue() +
                    Ext.getCmp(prototype.id + '-cmbDateFromDay').getValue();

            beanDetAll.IN_FECHA_TO = Ext.getCmp(prototype.id + '-cmbDateToYear').getValue() +
                    Ext.getCmp(prototype.id + '-cmbDateToMonth').getValue() +
                    Ext.getCmp(prototype.id + '-cmbDateToDay').getValue();
...
//                this.obtainComboFuente();
    ...
    searchDataDetalle_clickHandler: function (obj, metaData, rowNum, columnNum, obj2, rowData) {
        var beanDetAll = rowData.data;
        console.log(rowData.data);
        beanDetAll.INPNAME = rowData.data.FUENTE;
        beanDetAll.DTRANS = rowData.data.FECHA;
        beanDetAll.FECR = rowData.data.FECHA;
