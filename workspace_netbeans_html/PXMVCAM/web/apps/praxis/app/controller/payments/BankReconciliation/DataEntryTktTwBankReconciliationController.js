Ext.define('Ext.Praxis.controller.payments.BankReconciliation.DataEntryTktTwBankReconciliationController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.DataEntryTktTwBankReconciliationController',
...
//                    }
    ...
    obtainData: function () {
        Ext.Ajax.request({
            ...
            failure: function (response, opts) {
                console.log('server-side failure with status code ' + response.status);
                Ext.getCmp(prototype.id + '-dataEntryTktTw').unmask();
            }
        });
    ...
    onSearchPendingDetail: function () {
        ...
        console.log(paramDetail);
        Ext.Ajax.request({
            url: prototype.url + '/searchBeantTktTW_SCAN_PENDING',
            method: 'POST',
            timeout: 60000000,
            params: paramDetail,
            beforerequest: Ext.getCmp(prototype.id + '-dataEntryTktTw').mask('Loading...'),
            success: function (response, opts) {
                Ext.getCmp(prototype.id + '-dataEntryTktTw').unmask();
                var res = Ext.JSON.decode(response.responseText);
                console.log(res);
...
//            datos = this.procesarRegistros(miGrilla);
            if (Array.isArray(datos) && datos.length === 0) {
                ...
                Ext.Ajax.request({
                    url: prototype.url + '/executeOption_TktTw',
                    method: 'POST',
                    timeout: 60000000,
                    ...
                    success: function (response, opts) {
                        ...
                        if (res.success) {
                            ...
                            global.Msg({
                                ...
                                fn: function () {
                                    //exito
                                    Ext.getCmp(prototype.id + '-dataEntryTktTw').close();
