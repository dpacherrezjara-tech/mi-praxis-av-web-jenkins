 ...
 */
Ext.define('Ext.Praxis.controller.salesaudit.SalesAuditAccepted.DataEntryDetailsReasonController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.DataEntryDetailsReasonController',
    BeanReason: {},
    ...
    cargaDatos: function () {
        ...
        Ext.getCmp(prototype.id5 + '-griddata').getStore().removeAll();
        var mask = new Ext.LoadMask(Ext.getCmp(prototype.id5 + '-form'), {
            msg: 'Please Wait....'
        });
        mask.show();
        Ext.Ajax.request({
            url: this.urlWin01 + '/searchLstRazones',
            ...
            success: function (records, operation, success) {
                ...
                if (res.data.length > 0) {
                    Ext.getCmp(prototype.id5 + '-griddata').getStore().loadData(res.data);
                } else {
    ...
    setStoresGrids: function () {
        var grid01 = Ext.getCmp(prototype.id5 + '-griddata');
