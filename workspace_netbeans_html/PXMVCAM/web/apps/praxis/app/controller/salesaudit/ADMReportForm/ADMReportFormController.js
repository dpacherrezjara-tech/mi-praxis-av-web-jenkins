...
Ext.define('Ext.Praxis.controller.salesaudit.ADMReportForm.ADMReportFormController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.ADMReportFormController',
    ...
    onRendererColumnNotice: function (value, metaData, record, rowIndex, colIndex, store, view) {
        switch (String(Ext.String.trim(record.get('A2548STCOR')))) {
            case 'P':
                value = 'Sent';
                break;
            case 'Y':
    ...
    imgSearch_clickHandler: function (obj, e) {
        ...
        var txtEPR = Ext.getCmp(prototype.id + '-txtEPR').getValue();
        var txtPNR = Ext.getCmp(prototype.id + '-txtPNR').getValue();
        ...
        if (ComboBy === "2" || ComboBy === "3" || ComboBy === "4" || ComboBy === "15") {
            this.bean.COMBOBY = CmbProcess;
            this.bean.OPCIONTYPE = ComboBy;
            if (ComboBy === "2" || ComboBy === "3" || ComboBy === "15")
            {
        ...
        this.bean.VP_EPR = txtEPR;
        this.bean.VP_PNR = txtPNR;
        /*
    ...
    img_clickHandler_save: function () {
        ...
        if (grid.getSelectionModel().hasSelection()) {
            ...
            for (var i = 0; i < selection.length; i++) {
                ...
                if ((Ext.String.trim(row.get('A2548FTE')) !== 'ASR' || Ext.String.trim(row.get('A2548FTE')) !== 'MAN') && (Ext.String.trim(row.get('A2548BASE')) !== 'MS' || Ext.String.trim(row.get('A2548BASE')) !== 'UP' || Ext.String.trim(row.get('A2548BASE')) !== 'BF' || Ext.String.trim(row.get('A2548BASE')) !== 'MP')) {
                    Ext.Msg.alert('.: PRAXIS :.', 'The single process applies to the ASR or MAN source');
                    return;
                } else {
                    if (Ext.String.trim(row.get('A2548FLAG')) !== 'D') {
                        global.Msg({msg: 'The ticket status must be in disputado'});
                        return;
                    } else {
