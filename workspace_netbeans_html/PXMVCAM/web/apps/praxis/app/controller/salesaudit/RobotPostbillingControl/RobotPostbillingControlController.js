...
Ext.define('Ext.Praxis.controller.salesaudit.RobotPostbillingControl.RobotPostbillingControlController',{
    extend: 'Ext.app.ViewController',
    alias: 'controller.RobotPostbillingControlController',
     bean: {},
    bean2: {},
    ...
    setStores: function() {
        ...
        var store01 = Ext.create('Ext.data.Store', {
            proxy: {
                type: 'ajax',
                url: prototype.url + '/SearchPostbillingControl/',
                timeout: '300000',
                reader: {
                    type: 'json',
                    rootProperty: 'data',
    ...
    setStoresFilters: function() {
        ...
        var ComboTurno = Ext.getCmp(prototype.id + '-ComboTurno');
        ...
        cmbRobot.bindStore(Ext.create('Ext.data.Store', {
         ...
         ComboTurno.bindStore(Ext.create('Ext.data.Store', {
            data: [
                {"code": "", "name": "ALL"},
                {"code": "TA", "name": "Afternoon"},
                {"code": "MA", "name": "Morning"},
                {"code": "NO", "name": "Night"}
            ]
        }));
    },
    ...
    imgSearch_clickHandler: function(obj, records, eOpts) {
        ...
        var ComboTurno = Ext.getCmp(prototype.id + '-ComboTurno').getValue();
        if (cmbsearch == '') {
        ...
        if (bExcel) {
            ...
            Ext.getCmp(prototype.id + '-gridData').getStore().loadPage(1, {
                params: {
                    ...
                    if (records.length != 0) {
                        Ext.getCmp(prototype.id + '-lbl-total').setText(records[0].data.A3536TOTALPAG);
                    } else {
                        Ext.getCmp(prototype.id + '-lbl-total').setText('0');
    ...
    searchform_detalle_RFND: function(rowIndex) {
        ...
        if (rec.data.A3536FINA === 'A') {
            ...
            this.bean2.IN_DATEFROM = rec.data.A3536FREGI;
            this.bean2.IN_COUNTRY = rec.data.A3536PAIS;
            ...
            Ext.getCmp(prototype.id + '-gridDetalle').getStore().loadPage(1, {
                params: {
                    ...
                    if (records.length != 0) {
                        Ext.getCmp(prototype.id + '-lbl-totalDeta').setText(records[0].data.A3536TOTALPAG);
                    } else {
                        Ext.getCmp(prototype.id + '-lbl-totalDeta').setText('0');
                        global.Msg({msg: "Data not found.", icon: 2, fn: function() {
                            }});
