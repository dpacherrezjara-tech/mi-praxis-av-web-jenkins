...
Ext.define('Ext.Praxis.controller.salesaudit.SpdrspcrControl.SpdrspcrControlController',{
    extend: 'Ext.app.ViewController',
    alias: 'controller.SpdrspcrControlController',
    ...
    onRendererColumnOnTime: function(value, metaData, record, rowIndex, colIndex, store, view){
        switch( String(record.get('A3540STATO')) ){
            case 'D':
                value = 'silver';
            break;
            case 'A':
    ...
    OnDetail01: function(rowIndex) {
       ...
       if (rec.data.A3540STATO === 'A') {
            ...
            this.bean2.IN_DATEFROM = rec.data.A3540FREGI;
            this.bean2.IN_COUNTRY = rec.data.A3540PAIS;
            this.bean2.IN_ROBOT = rec.data.A3540TYPE;
            ...
            Ext.getCmp(prototype.id + '-gridDetalle').getStore().loadPage(1, {
                params: {
                    ...
                    if (records.length != 0) {
                        Ext.getCmp(prototype.id + '-lbl-totalDeta').setText(records[0].data.A3540CANT);
                    } else {
        ...
        if (bExcel) {
            ...
            Ext.getCmp(prototype.id + '-gridData').getStore().loadPage(1, {
                params: {
                    ...
                    if (records.length != 0) {
                        Ext.getCmp(prototype.id + '-lbl-total').setText(records[0].data.A3540TOTALPAG);
                    } else {
