...
Ext.define('Ext.Praxis.controller.gerencial.BiTools.DataEntrySaveController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.' + prototype.id + '-dataEntrySaveController',
    url: CONTEXTPATH + '/BiTools',
    dataGuia: '',
    meDe: '',
    p: {},
    params: {},
...
//            } else {
            ...
            for (var k = 0; k < cbxQuerysAC.length; k++) {
                if (cbxQuerysAC[k].data.code.trim() === txtNameQuery.trim()) {
                    exist = true;
                }
            }
...
//        }
        ...
        meDe.params.IN_TABLA = tabla;
        meDe.params.IN_TABLA2 = '';
...
//                    Ext.getCmp(prototype.id + '-txtDescQuery').setValue(prototype.getDatoCombo('-cmbFav', 'code', 'helper'));
//                }
        ...
        Ext.Ajax.request({
            ...
            success: function(response, options) {
                ...
                for (var i = 0; i < res.length; i++) {
                    var datos = {};
                    datos = {
                        code: res[i].strCodigo,
                        name: res[i].strCodigo + ' - ' + res[i].strDescrip,
