Ext.define('Ext.Praxis.controller.screens.ScrLegController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.ScrLegController',
    meEle: "",
    p: '',
    bean: {},
    modBack: '',
    ...
    init: function(view) {
        ...
        prototype.Leg = {
            id: 'ScrLegForm',
            url: CONTEXTPATH+'/ScrLeg'
        };
    },
    afterRender: function() {
        this.startDisplay();
        //console.log(this.p);
        this.loadTicketLegs(this.p.bean.IN_CCUST, this.p.bean.IN_CIA, this.p.bean.IN_FORMA, this.p.bean.IN_SERIE,this.p.bean.IN_CUPON);
    },
    ...
    loadTicketLegs: function (IN_CCUST, IN_CIA, IN_FORMA, IN_SERIE,IN_CUPON) {
        console.log(IN_CCUST);
        Ext.Ajax.request({
            url: prototype.Leg.url+'/loadTicketLegs',
            method: 'POST',
            timeout: 60000000,
            params: {IN_CCUST: IN_CCUST, IN_CIA: IN_CIA, IN_FORMA: IN_FORMA, IN_SERIE: IN_SERIE, IN_CUPON: IN_CUPON},
            beforerequest: Ext.getCmp('ScrLegForm').mask('Loading...'),
            success: function (response, opts) {
                var res = Ext.JSON.decode(response.responseText);
                if (res.success) {
                    meEle.onResultLoadTicketLegs(res);
                } else global.Msg({msg: res.sesion});
                Ext.getCmp('ScrLegForm').unmask();
            },
