Ext.define('Ext.Praxis.controller.program.ProBankReconciliationTktController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.ProBankReconciliationTktController',
    childs: '',
    stack: [],
    bean: {},
    lstConditions: [],
...
//                Ext.getCmp(prototype.MatchTkt.id+'-cmbDateMonth').setValue());
            ...
            if(this.bean.IN_CARDC !== ''){
                this.buscarCampoLibre("SCARCOD", "EQ", this.bean.IN_CARDC);
            }
            if(this.bean.IN_MERCHN !== ''){
                this.buscarCampoLibre("MERCHN", "EQ", this.bean.IN_MERCHN);
            }
            if(this.bean.IN_COUNTRY !== ''){
                this.buscarCampoLibre("SCOUNTRY", "EQ", this.bean.IN_COUNTRY);
            }
    ...
    matchManual: function (bean) {
        Ext.Ajax.request({
            ...
            beforerequest: Ext.getCmp('ProBankReconciliationTktForm').mask('Loading...'),
            success: function (response, opts) {
                Ext.getCmp('ProBankReconciliationTktForm').unmask();
                win.lblUser_toolTip("Estructura: A2291");
                var res = Ext.JSON.decode(response.responseText);
//                if (res.success) {
