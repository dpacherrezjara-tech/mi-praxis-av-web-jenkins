...
Ext.define('Ext.Praxis.controller.sales.ProcessingCommissions.ProcessingCommissionsController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.ProcessingCommissionsController',
    fecha: new Date(),
    nameText: '',
    nameText2: '',
    me: '',
    ...
    init: function(view) {
        ...
        this.control({
            // -------------------Eventos Genericos --------------------
            '#ProcessingCommissionsForm-xpanel': {
                afterrender: this.xpanel_afterrender
            },
            '#ProcessingCommissionsForm-btnProccess': {
                click: this.btnProccess_click
            },
            '#ProcessingCommissionsForm-cbxType': {
                change: this.onChangeType
            },
            '#ProcessingCommissionsForm-cbxZona': {
                change: this.onChangeZone
            },
            '#ProcessingCommissionsForm-btnDownloadFile': {
                click: this.onDowloadFile
            },
            '#ProcessingCommissionsForm-btnDownloadFile2': {
                click: this.onDowloadFile2
            }
        });
    },
    xpanel_afterrender: function(obj, e) {
        Ext.getCmp(prototype.id + '-cbxPeriodicity').hide();
        Ext.getCmp(prototype.id + '-cbxInvoice').hide();
        Ext.getCmp(prototype.id + '-cbxPerio').hide();
        Ext.getCmp(prototype.id + '-cbxZona').hide();
    ...
    onChangeType: function(obj, value) {
        ...
        Ext.getCmp(prototype.id + '-cbxPerio').hide();
        if (value === 'GSA') {
            Ext.getCmp(prototype.id + '-cbxPeriodicity').show();
            Ext.getCmp(prototype.id + '-cbxInvoice').show();
        }
    ...
    onDowloadFile: function() {
        ...
        if (msj.trim() === '') {
            ...
            var strType = Ext.getCmp(prototype.id + '-cbxType').getValue();
            var strZona = Ext.getCmp(prototype.id + '-cbxZona').getValue();
            ...
            Ext.Msg.show({
                ...
                fn: function(btn) {
                    if (btn === 'yes') {
                        global.getFile(prototype.url + '/getFileTxt/?strType='+strType+'&strZona='+strZona+'&nameLote='+nameLote+'&nameText=' + name);
                    }
                }
            });
...
//            });
    ...
    setStoreData: function() {
        var cbxInvoice = Ext.getCmp(prototype.id + '-cbxInvoice');
        var cbxPeriodicity = Ext.getCmp(prototype.id + '-cbxPeriodicity');
        var cbxType = Ext.getCmp(prototype.id + '-cbxType');
        var cbxPerio = Ext.getCmp(prototype.id + '-cbxPerio');
        cbxInvoice.bindStore(Ext.create('Ext.data.ArrayStore', {
            autoLoad: true,
            fields: ['code', 'name'],
            data: [
                ["", "Select"],
        ...
        cbxType.setValue("");
        cbxPeriodicity.bindStore(Ext.create('Ext.data.ArrayStore', {
            autoLoad: true,
            fields: ['code', 'name'],
            data: [
                ["", "Select"],
        ...
        cbxPeriodicity.setValue("");
        cbxPerio.bindStore(Ext.create('Ext.data.ArrayStore', {
            autoLoad: true,
            fields: ['code', 'name'],
            data: [
                ["", "Select"],
        ...
        Ext.Ajax.request({
            ...
            success: function(response, options) {
                ...
                Ext.getCmp(prototype.id + '-cbxZona').bindStore(storeData);
            }
        });
    },
    validateParams: function() {

        var periodicity = Ext.getCmp(prototype.id + '-cbxPeriodicity').getValue();
        var invoice = Ext.getCmp(prototype.id + '-cbxInvoice').getValue();
        var type = Ext.getCmp(prototype.id + '-cbxType').getValue();
        var perio = Ext.getCmp(prototype.id + '-cbxPerio').getValue();
        var msj = '';
        if (type === 'GSA') {

            if (invoice === '') {
                msj = 'Selected Invoice Type';
                return msj;
            }
            if (invoice === 'I' && periodicity === '') {
                msj = 'Selected Periodicity Type';
                return msj;
            }
        }
        else if (type === 'FOB') {
            //if(perio=='') mensaje = 'Selected Type';
        }
        else {
            msj = 'Selected Type';
            return msj;
    ...
    validateDowload: function() {
        ...
        var type = Ext.getCmp(prototype.id + '-cbxType').getValue();
        var zona = Ext.getCmp(prototype.id + '-cbxZona').getValue();
        var msj = '';
        if (type === 'GSA') {
            if (zona === '') {
                msj = 'Selected Zone';
    ...
    procesarResultado: function(status) {
        ...
        if (status === 'C') {
            if (type === 'GSA') {
                Ext.getCmp(prototype.id + '-cbxZona').show();
                Ext.getCmp(prototype.id + '-panelBtn').hide();
            } else {
