Ext.define('Ext.Praxis.controller.sales.InvoiceCommissionFOB.LoadFileController',{
    ...
    afterRender: function(){
        ...
        switch( this.p.action ){
            case 'I':
                Ext.getCmp(prototype.idLoadFileFOB+'-btn-upload').show();
                break;
        }
    },
    onUploadChange: function(cmp, value) {
        ...
        if (this.file !== undefined) {
            ...
            if (cmp.getErrors().length>0) Ext.getCmp(prototype.idLoadFileFOB+'-btn-upload').disable(true);
            else Ext.getCmp(prototype.idLoadFileFOB+'-btn-upload').enable(true);
        } else Ext.getCmp(prototype.idLoadFileFOB+'-btn-upload').disable(true);
    },
    ...
    onInsertClick: function() {
        Ext.Msg.show({
            ...
            fn: function(btn) {
                if (btn === 'yes') {
                    Ext.getCmp(prototype.idLoadFileFOB+'-btn-upload').disable(true);
                    this.setData();
                }
            }
        });
    ...
    setData: function() {
        var me = this;
        var file = Ext.getCmp(prototype.idLoadFileFOB + '-file').getValue();
        if (file === '') {
            Ext.MessageBox.alert('PRAXIS', "Select File", function (btn, text) {
                if (btn === 'ok' || btn === 'cancel')
                    setTimeout("Ext.getCmp(prototype.idLoadFileFOB + '-File').focus();", 100);
            });
            return;
        }    
        me.searchParams.VP_ACTION = 'I';
        me.searchParams.VP_A1757CCUST = '139';
        me.searchParams.fileName = file;
        var form = Ext.getCmp(prototype.idLoadFileFOB + '-form-01').getForm();
        form.submit({
            ...
            success: function (fp, o) {
                ...
                Ext.getCmp(prototype.idLoadFileFOB+'-btn-upload').enable(true);
            },
    ...
    getValue: function(id) {
        return Ext.getCmp(prototype.idLoadFileFOB+'-'+id).getValue();
    },
    focus: function(id) {
        Ext.getCmp(prototype.idLoadFileFOB+'-'+id).focus();
    },
    setValue: function(id, txt) {
        Ext.getCmp(prototype.idLoadFileFOB+'-'+id).setValue(txt);
    },
