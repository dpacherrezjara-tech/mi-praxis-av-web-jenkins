Ext.define('Ext.Praxis.controller.interline.EstimationReverseAP.EstimationReverseAPController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.EstimationReverseAPController',
    me: '',
    childs: '',
    bean: {},
    lstType: new Array(),
...
//    _path: '',
    ...
    imgSearch_clickHandler: function(obj, e) {
        this.bean.IN_A2134FPROC = this.getValue("cmbDateYear") + this.getValue("cmbDateMonth") + this.getValue("txtPeriodo");
	this.bean.IN_A2134FPROC = this.bean.IN_A2134FPROC === "000" ? "" : this.bean.IN_A2134FPROC;
	this.bean.IN_A2134FUENT = this.getValue("cbxSource");
	this.bean.IN_A2134SFUEN = this.getValue("txtChannel");
	this.bean.IN_A2134PSVTA = this.getValue("txtCountry");
        
        if (typeof obj == 'string') this.mensajeResult = obj;
        else this.mensajeResult = "";
    ...
    imgEstimado_clickHandler: function () {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if(objA2134.POLIZA) lstEstimado.push(objA2134);
        }
    ...
    imgReversa_clickHandler: function () {
        ...
        for (var i = 0; i < gridDataSource.length; i++) {
            ...
            if(objA2134.A2134STPRO==='0' &&  objA2134.A2134STS0==='Y' && objA2134.A2134SPROC==='C') lstReversa.push(objA2134);
        }
        if (lstReversa.length > 0) {
            Ext.create('Ext.Praxis.view.interline.EstimationReverseAPForm.DataEntryReverse', {
                id: 'DataEntryReverseEstimationReverseAPForm',
                params: {
                    actionCode: 'I',
...
//            this.deleteFavoriteMenu(this.bean2149);
    ...
    btnDownloadFiles_click: function () {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if(obj.DESCAPGA) this.lstDownload.push(obj);
        }
        if (data.length > 0) {
        ...
        var storeGridDatas = Ext.create('Ext.Praxis.store.interline.GridData', {
            ...
            listeners: {
                ...
                load: function (obj, obj2, success, obj4, obj5) {
                    win.lblUser_toolTip("Estructura: A2134");
    ...
    listarCuentasA1740: function (action) {
        Ext.Ajax.request({
			...
			obj.A1740CLIE = dato.A1740CLIE;
                    ...
                    if (action === 'I') me.loadGrupo(action);
                    else if (action === 'U') me.loadCuentas(action, me.beanDetail);
        ...
        Ext.Ajax.request({
            ...
            success: function (response, opts) {
                ...
                if (res.success) {
                    var lstCuentas = res.lstCuentas;
                    Ext.getCmp(prototype.id + '-centerC').unmask();
                    Ext.create('Ext.Praxis.view.interline.EstimationReverseAPForm.DataEntry', {
                        id: 'DataEntryEstimationReverseAPForm',
                        params: {
                            actionCode: action,
                            bean: beanDetail,
                            lstConta: lstCuentas,
